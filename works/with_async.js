...
var main_msg_listener = async (value) => {
    ...
    remote_print(new Date().toString());
    try {
        if (this_value === -1) {
            returned_value = -1;
            ...
            // const response = await fetch(base_url + '/minmax');
            // if (!response.ok) {
            ...
            // // 将Base64编码的字符串转为字节数组
            // const response_data = base64ToArrayBuffer(responseText);
            // const responseArray = new Uint8Array(response_data);
            // // 设置共享字节
            // shared_bytes.set(responseArray);
            // // 设置返回值
            returned_value = -2;
